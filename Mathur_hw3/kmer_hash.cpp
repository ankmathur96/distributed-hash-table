...
int main(int argc, char **argv) {
  ...
  if (run_type == "verbose") {
    BUtil::print("With %d nodes, initializing hash table of size %d and %d per node for %d kmers.\n",
      upcxx::rank_n(), hash_table_size, n_per_node, n_kmers);
  }
  ...
  upcxx::barrier();
  if (run_type == "verbose") {
    BUtil::print("Hashmap owning rank: %d\n",
      upcxx::rget(hashmap.owning_rank).wait());
  }
  ...
  for (auto &kmer : kmers) {
    BUtil::print("Before hmap insert\n");
    bool success = hashmap.insert(kmer);
    BUtil::print("After hmap insert\n");
    if (!success) {
