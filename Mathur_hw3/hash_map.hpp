...
uint64_t HashMap::compute_segment_index(uint64_t i) {
  ...
  std::cout << "Compute segment index:" << i << ", data offset: " << compute_data_offset(i) << ", per_node_size: " << per_node_size<<"\n" << std::flush;
...
void HashMap::lock_acquire(uint64_t slot) {
  ...
  while (!l_acquired) {
    if (upcxx::rget(locks + compute_data_offset(slot)).wait() == 0) {
      // acquire lock on the segment
      uint64_t acquire_val = 1.0;
      upcxx::rput(acquire_val, locks + compute_data_offset(slot)).wait();
      return;
    }
  }
}
...
bool HashMap::slot_used(uint64_t slot) {
  ...
  upcxx::future<int> used_fut = upcxx::rget(segment_used_array + compute_segment_index(slot));
  used_fut.wait();
  if (used_fut.result() != 0) {
    return true;
  } else {
    return false;
  }
...
kmer_pair HashMap::read_slot(uint64_t slot) {
  upcxx::global_ptr<kmer_pair> segment_array = upcxx::rget(data+compute_data_offset(slot)).wait();
  upcxx::future<kmer_pair> data_fut = upcxx::rget(segment_array + compute_segment_index(slot));
  data_fut.wait();
  return data_fut.result();
}

bool HashMap::request_slot(uint64_t slot) {
  std::cout << "Requesting Slot:" << slot << "\n" << std::flush;
  upcxx::global_ptr<int> segment_used_array = upcxx::rget(used+compute_data_offset(slot)).wait();
  lock_acquire(slot);
  std::cout << "Lock acquired. Slot:" << slot << "\n" << std::flush;
  int* used_array = segment_used_array.local();
  std::cout << "Segment used obtained. Slot:" << slot << "\n" << std::flush;
  std::cout << "Segment Index: " << compute_segment_index(slot) << ", Slot: " << slot << "\n" << std::flush;
  int used_val = used_array[compute_segment_index(slot)];
  std::cout << "Used Val: " << used_val << ", Slot:" << slot << "\n" << std::flush;
  if (used_val != 0) {
    return false;
  } else {
    upcxx::rput(1, segment_used_array + compute_segment_index(slot)).wait();
    std::cout << "Slot granted: " << slot << "\n" << std::flush;
    return true;
  }
}
